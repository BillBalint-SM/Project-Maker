  ...
  it('normalizes paged mailbox changes and preserves replay for the same checkpoint', async () => {
    ...
    client.pages.set('initial', {
      value: [{
        ...
        toRecipients: [
          { emailAddress: { address: 'project-maker+opaque-token@pte.hu' } },
          { emailAddress: { address: 'PROJECT-MAKER+OPAQUE-TOKEN@PTE.HU' } },
        ],
        ccRecipients: [{ emailAddress: { address: 'observer@example.test' } }],
        subject: 'Re: Kérdésséma',
        body: { contentType: 'text', content: 'Válasz' },
        receivedDateTime: '2026-08-17T08:00:00.000Z',
        attachments: [
          { name: 'scope.pdf', contentType: 'application/pdf', size: 1280 },
        ],
      }],
    ...
    assert.deepEqual(first, {
      changes: [{
        ...
        recipientAddresses: [
          'project-maker+opaque-token@pte.hu',
