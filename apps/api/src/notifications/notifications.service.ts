    ...
    const items: InternalNotification[] = [
      ...due.map((row): InternalNotification => {
        ...
        return {
          ...
          reason: overdue ? 'A következő lépés határideje lejárt.' : 'A következő lépés hét napon belül esedékes.',
      ...
      ...replies.map((row) => ({
        ...
        projectId: row.project_id,
        projectName: row.project_name,
        reason: 'Új ügyfélválasz vár feldolgozásra.',
      ...
      ...responses.map((row) => ({
        ...
        projectId: row.project_id,
        projectName: row.project_name,
        reason: 'Új ügyfél-pontosítás érkezett és átnézésre vár.',
        attentionAt: new Date(row.attention_at).toISOString(),
        actionUrl: `/projects/${row.project_id}/customer-correspondences`,
      })),
      ...failures.map((row) => ({
        ...
        projectId: row.project_id,
        projectName: row.project_name,
        reason: 'Egy ügyfélküldés eredménye hibás vagy bizonytalan.',
        attentionAt: new Date(row.attention_at).toISOString(),
        actionUrl: `/projects/${row.project_id}/customer-correspondences`,
      })),
    ];
