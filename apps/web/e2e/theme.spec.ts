...
test('uses the viewport width exposed by browser zoom-out', async ({ page }) => {
  await page.route('**/api/auth/session', async (route) => {
    await route.fulfill({
      contentType: 'application/json',
      body: JSON.stringify({
  ...
  await page.setViewportSize({ width: 5120, height: 1600 });
  await page.goto('/');
  await expect(page.getByRole('heading', { name: 'Project Journey' })).toBeVisible();

  const geometry = await page.evaluate(() => {
    const viewportWidth = document.documentElement.clientWidth;
    const headerWidth = document.querySelector<HTMLElement>('.header-inner')!.getBoundingClientRect().width;
    const mainWidth = document.querySelector<HTMLElement>('.app-main')!.getBoundingClientRect().width;
    return { headerWidth, mainWidth, viewportWidth };
  });

  expect(geometry.headerWidth / geometry.viewportWidth).toBeGreaterThan(0.95);
  expect(geometry.mainWidth / geometry.viewportWidth).toBeGreaterThan(0.95);

  await page.getByTestId('navigation-toggle').click();
  const navigationPanel = page.getByTestId('navigation-panel');
  await expect(navigationPanel).toHaveClass(/open/);
  await expect(navigationPanel).toBeVisible();
  const navigationWidth = await navigationPanel.evaluate(
    (element) => element.getBoundingClientRect().width,
  );
  expect(navigationWidth / geometry.viewportWidth).toBeGreaterThan(0.95);
});
