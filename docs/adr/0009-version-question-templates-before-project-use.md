...
**Consequences:** A template must be published before application. Project creation in the internal UI requires an available published template, while older API clients remain readable and compatible. Publishing or application fails if a selected stable key is missing or inactive in the latest Question Bank. Changing or logically deleting a template never changes an existing Project schema or interview: accepted schemas retain the exact wording, ordering, required/blocking policy, Question Bank version, and template provenance that participants actually answered.
