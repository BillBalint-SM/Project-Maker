...
- **Customer email follow-up:** an outbound communication cadence/schedule. It may send pings, but it is not a discovery work item and does not replace discovery follow-ups. Every logical manual or scheduled ping owns one immutable Microsoft 365 outbound communication, central tokenized Reply-To and Customer correspondence. Scheduled workers claim due work through PostgreSQL before Graph submission and revalidate the current recipient, draft, and optional reference. Known rejection is `FAILED`; indeterminate acceptance is `UNKNOWN` and pauses the cadence. Neither state retries automatically. Explicit retry preserves the logical ping identity; a later ping creates new identities. `UNKNOWN` additionally requires request-specific duplicate-risk acknowledgement.
