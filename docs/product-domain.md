...
- **Follow-up:** an unresolved question with a responsible owner, due date, status, answer/decision, and next step.
...
Each follow-up preserves a stable `id`, optional source checklist item ID, `category`, `question`, `owner`, `dueDate`, `status`, `decisionOrAnswer`, and `nextStep`.
