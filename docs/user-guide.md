...
- [Felmérés lezárása és ügyfélcsomag](#felmérés-lezárása-és-ügyfélcsomag)
- [Felkészültségi értékelés és hiányok](#felkészültségi-értékelés-és-hiányok)
- [Tisztázási tételek kezelése](#tisztázási-tételek-kezelése)
...
- a vezetett kezdő felmérés mentett válaszait és verziózott ügyfélcsomagjait;
...
| Befejeződik a felmérés | Zárd le akkor is, ha maradt hiány, majd küldd az ügyfélcsomagot most vagy később | Verziózott piszkozat készül; a hiányok a felkészültségben maradnak láthatók |
...
| Új bizonytalanság merült fel | Hozz létre tisztázási tételt felelőssel és dátummal | A tisztázandó pont számonkérhetően megmarad |
...
A `Kérdésbank beállítások` menüpont technikailag nincs adminszerephez kötve. Szervezetileg mégis csak a kijelölt kérdésbank-gazda használja, mert egyetlen mentés minden későbbi projektsémára ható új bankverziót hoz létre.
...
- Ne írj jelszót, hozzáférési tokent, privát kulcsot vagy más titkot válaszba, tisztázási tételbe, Markdownba vagy mezőbe.
...
- `Áttekintő`: az aktív projektek és minden projektkörnyezet bejárata;
- `Új projekt`: új ügyfélprojekt rögzítése;
- `Folyamatban lévő ügyek`: a következő projektfeladatok priorizált munkasora;
- `Utánkövetések`: az összes aktív projekt nyitott tisztázási tétele;
- `Markdown beállítások`: a Markdown-kimenet szervezeti sablonja;
- `Kérdésbank beállítások`: a szervezeti szintű közös kérdésbank.

![Az Áttekintő asztali nézete a hat állandó navigációs ponttal és egy mintaprojekttel](assets/user-guide/11-project-work-hub-desktop.png)

*Az asztali Áttekintőn az ügyfélválaszok száma közvetlenül a szűrt munkasorba vezet. A képen kizárólag fiktív mintaadat szerepel.*

![Az Utánkövetések keskeny nézete egymás alá rendezett mezőkkel és elérhető elsődleges művelettel](assets/user-guide/12-project-work-hub-narrow.png)
...
Egy projekten belül a közös fejléc és a visszalépő link őrzi a munkafolyamatot: a visszalépés pontosan az `Áttekintő`, a `Folyamatban lévő ügyek` vagy az `Utánkövetések` korábbi állapotába visz, a projektfülek pedig ugyanabban a projektkörnyezetben maradnak.

| Felület | Mire való? | Legfontosabb műveletek |
| --- | --- | --- |
| `Áttekintő` | Aktív projektek áttekintése | Új projekt, következő feladat megnyitása |
...
| `Felkészültség` | Felkészültségi értékelés és tisztázások | Hiányok javítása, tisztázási tételek létrehozása és lezárása |
...
| `Felmérés` | Projektséma, kezdő felmérés és ügyfélcsomag | Séma elfogadása és első kör indítása, válaszadás, felmérés lezárása, előnézet és küldés |
| `Markdown terv` | Változatlan kanonikus specifikációk | Revízió generálása, összehasonlítás, előnézet, letöltés |
| `Kérdésbank beállítások` | Minden projekt közös kérdéskészlete | Kérdés létrehozása, új verziót létrehozó szerkesztés |
...
flowchart LR
    ...
    H --> I
    I --> J[Tisztázási tételek lezárása]
...
A felmérés értékelése után nyisd meg a projekt `Felkészültség` oldalát. A felmérés közben felmerülő, később megválaszolandó pontokat ne rejtsd el egy hosszú válaszban. Hozz létre külön tisztázási tételt kategóriával, felelőssel, valódi céldátummal és következő lépéssel. Lezáráskor a döntést vagy választ is rögzítsd.

### 5. Pillanatkép és kommunikáció

Belső, kézi munkához generálhatsz friss Markdown-revíziót, majd megnézheted a forráspillanatképet és az előnézetet. A Project Makerben automatizált Claude Code átadás még nem érhető el, és a `.md` fájl nem ügyfélkommunikáció. Ügyfélnek a felmérési összefoglalót vagy a külön megírt ügyfél-emlékeztetőt küldd; egyik levél sem csatolja és nem másolja be a Markdown-revíziót.
...
- A `Kérdésbank beállítások` oldalon van legalább egy aktív kérdés.

### Lépések

1. Nyisd meg az `Áttekintő` oldalt, és válaszd az `Új projekt` gombot.
...
4. A `Projektbeállítások` oldalon állítsd az életciklus-állapotot `Felmérés folyamatban` értékre.
...
10. A felmérés végén válaszd a `Felmérés lezárása és továbblépés` vagy a `Lezárás és ügyfélcsomag előnézete` műveletet. A lezáráshoz nem kell minden üzleti hiányt kitölteni, de függőben lévő vagy hibás mentés nem maradhat.
...
13. Nyisd meg a `Felkészültség` oldalt. Minden későbbi tisztázandó pontból hozz létre külön tisztázási tételt.
14. Átadás előtt nyisd meg a `Markdown terv` oldalt, és válaszd a `Markdown-revízió generálása` gombot.
15. Ellenőrizd a `Tartalmi előnézet` részt és a revízió metaadatait.
...
Az `Áttekintő` oldalon minden projekt egy kártya. A kártya megmutatja:

- a projekt nevét;
- az aktuális életciklus-állapotot;
...
A lista a projekt saját koordinációs vagy életciklus-mentésének legutóbbi ideje szerint rendezi előre a kártyákat. Egy felmérési válasz vagy tisztázási tétel önmagában nem feltétlenül mozgatja előre a projektet.
...
![A Folyamatban lévő ügyek nézete három, sürgősség szerint rendezett fiktív projekttel, szűrőkkel és elsődleges műveletekkel](assets/user-guide/10-active-project-queue.png)

*A Folyamatban lévő ügyek nézete a teljes portfólió következő teendőit csoportosítja; minden sor egy projektet és annak egyetlen elsődleges következő műveletét mutatja.*

Az Áttekintő fejlécében válaszd a `Folyamatban lévő ügyek` gombot, ha nem egy előre kiválasztott projektből, hanem az összes aktív projekt közül szeretnéd eldönteni, mivel foglalkozz következőként. A csoportok mindig ebben a sürgősségi sorrendben jelennek meg:
...
- ha a szűrők mellett nincs találat, a `Szűrők törlése` visszaállítja a teljes munkasort; ha egyáltalán nincs aktív projekt, innen visszatérhetsz az Áttekintőre vagy új projektet hozhatsz létre.
...
Siker esetén a webapp létrehoz egy `Előkészítés alatt` állapotú projektet, és közvetlenül a következő szükséges feladatra vezet. A projekt már az Áttekintőn is látható.
...
| `Felmérés folyamatban` | Aktív igényfelmérés vagy workshop folyik | Nem jelenti, hogy minden kérdésnek van válasza |
...
| `Becslésre kész` | A csapat üzletileg tervezésre késznek jelöli | Nem formális jóváhagyás és nem felkészültségi tanúsítvány |
| `Archivált` | Az aktív követés lezárt vagy szünetel, a történet megmarad | Nem törlés; visszaállítható |

A `Becslésre kész` állapotba lépés automatikusan létrehoz egy `Mérföldkő elérése` okú Markdown-revíziót `Becslésre kész` mérföldkőnévvel. A revízió változatlan marad; téves állapotválasztást újabb helyes állapottal és szükség esetén új revízióval korrigálj.
...
> **Szervezeti felelősség:** a `Kérdésbank beállítások` oldal technikailag minden alkalmazás-hozzáféréssel rendelkező személy számára elérhető. Mégis csak a kijelölt kérdésbank-gazda módosítsa, mert minden sikeres létrehozás vagy szerkesztés új, közös bankverziót publikál.
...
A tisztázási tételek és ügyfél-utánkövetési beállítások jelenleg nem részei ennek a Markdown-forrásnak.
...
## Felmérés lezárása és ügyfélcsomag

![Egy lezárt, részben kitöltött felmérés első ügyfélcsomag-piszkozata címzettel, előnézettel, küldési művelettel és verzióelőzménnyel](assets/user-guide/09-interview-customer-handoff.png)
...
- `Felmérés lezárása és továbblépés`: a felmérés lezárul, majd a `Felkészültség` oldal nyílik meg; az ügyfélcsomag később készíthető elő.
- `Lezárás és ügyfélcsomag előnézete`: a felmérés lezárul, majd megnyílik az előnézet. A küldés csak a megerősítés után indul.
...
1. Nyisd meg a korábban elküldött felmérési ügyfélcsomagot.
...
Az előző, elküldött verzió változatlanul megmarad. Egyszerre csak egy aktív, nem elküldött verzió lehet; ezért új verzió csak az előző sikeres küldése után indítható. Az új csomag nem külön felmérési kör, hanem ugyanannak a lezárt felmérésnek a következő, nyomon követhető átadási verziója.
...
*A `Felkészültség` oldalon látható értékelés a kanonikus kezdő felmérés aktuális állapotát, a tényezőket és a következő biztonságos javítási irányt mutatja; nem döntési pontszám és nem ajánlott döntés.*
...
- `Értékelési tényezők`: külön mutatják az alapinformációk, az üzleti tisztázottság, a felelősség, az ellenőrzőlista és a tisztázási utánkövetés állapotát.
- `Rendezendő hiányok`: a `Kritikus`, `Fontos`, majd `Pontosítás` sorrendben megjelenő, általánosított javítási jelzések. A lista nem jelenít meg felmérési választ, `Nem releváns` indoklást vagy tisztázási tétel tartalmát.

Minden hiány művelete a megfelelő munkafelületre vezet: a koordinációs hiány a `Projektállapot` szerkesztőjéhez, a kérdéshiány a megfelelő felmérési kérdéshez, a tisztázási hiány pedig ugyanazon `Felkészültség` oldal tisztázási listájához. Javítsd ott az adatot vagy zárd le a tételt, mentsd sikeresen, majd ellenőrizd a frissült értékelést.
...
**Előfeltétel a pontszámhoz:** mind a hat érték megvan, és a projekt aktuális kezdő felmérése a teljes, kanonikus sémából ad elérhető felkészültséget. Enélkül az oldal megmondja, hogy melyik feltétel hiányzik. A projektkoordináció és a tisztázási tételek ilyenkor is a megszokott módon szerkeszthetők.
...
## Tisztázási tételek kezelése

A tisztázási tétel olyan tisztázandó üzleti munkaelem, amelynek van egyértelmű kérdése, felelőse, céldátuma és következő lépése. Nem ugyanaz, mint az ügyfél-emlékeztető: előbbi belső projektmunka, utóbbi e-mail-küldési ütemezés.

![Egy megválaszolt üzleti és egy nyitott integrációs tisztázási tétel felelőssel, dátummal és következő lépéssel](assets/user-guide/05-discovery-follow-ups.png)

*A lista külön mutatja a végleges döntést és a még nyitott, szerkesztésre vagy lezárásra váró tisztázást.*

![Nyitott tisztázási tétel kompakt felmérési forráshivatkozással és annak kezelési műveleteivel](assets/user-guide/08-discovery-source-linkage.png)
...
### Új tisztázási tétel létrehozása

**Előfeltétel:** a projekt nem archivált, és nincs más tisztázási módosítás folyamatban.

1. A `Felkészültség` oldal `Tisztázási utánkövetések` részében válaszd ki a `Kategória` értékét.
...
7. Válaszd az `Utánkövetés létrehozása` gombot.
...
### Nyitott tisztázási tétel napi szerkesztése

Csak `Nyitott` tisztázási tétel szerkeszthető. A kívánt sorban válaszd a `Szerkesztés` gombot, majd szükség szerint módosítsd az öt munkamezőt: `Kategória`, `Tisztázandó kérdés`, `Felelős`, `Határidő` és `Következő lépés`. Az állapot, a végleges döntés vagy válasz és az elem azonosítója nem szerkeszthető.
...
Siker után a `Szerkesztés` és a `Lezárás` gomb is eltűnik, megjelenik a végleges állapot és a `Döntés vagy válasz`.

### Mi nem módosítható?

Lezárt (`Megválaszolva` vagy `Nem releváns`) tisztázási tétel nem szerkeszthető, nem nyitható újra, és a megőrzött forrása sem módosítható. Tételtörlés nem elérhető. Hibás, még nyitott kérdés, felelős vagy dátum esetén a `Szerkesztés` folyamatot használd; lezárt tételt ne próbálj hamis válasszal helyesbíteni. Ha valóban új tisztázandó kérdés keletkezik, hozz létre új tételt.

### Archivált projekt

Archiválás után a tisztázási lista és a kompakt forráshivatkozások olvashatók maradnak, de az új elem létrehozása, a `Szerkesztés`, a `Lezárás` és a forráshivatkozási műveletek letiltottak. Ha archiváláskor nyitva volt egy helyi szerkesztő-, lezárási vagy forráshivatkozási űrlap, illetve eltávolítási megerősítés, annak be nem mentett állapota törlődik. Visszaállítás után a projekt piszkozat állapotú lesz, a meglévő tételek megmaradnak, és a nyitott elemek műveletei újra elérhetővé válnak.
...
A projekt ügyfél-utánkövetése két összetartozó, de külön munkafelületű műveletből áll:
...
Mindkettő a projekt létrehozásakor rögzített `Ügyfél kapcsolattartó e-mail-címe` mező értékére küld. A címzett nem írható felül. A kézi küldés előtt a rendszer pontos előnézetben mutatja a címzettet, a tárgyat és a teljes egyszerű szöveges levelet. A Claude Code-nak szánt Markdown és a felmérési ügyfélcsomag nem része ennek a levélnek.

A felmérési összefoglaló az egyetlen teljes ügyfél-összefoglaló küldési folyamat. Az ügyfél-emlékeztető rövid, célzott levél: nem alternatív ügyfélcsomag, nem csatol Markdown-revíziót vagy `.md` fájlt, és nem továbbít belső Claude-instrukciót.
...
5. Válaszd az `Utánkövetési beállítások mentése` gombot.
...
Archivált projektben a mentett emlékeztető olvasható marad, de a szerkesztés, előnézet és küldés letiltott. Ha a munka valóban újraindult, előbb állítsd vissza a projektet, ellenőrizd a címzettet és a hivatkozott tisztázási tétel nyitott állapotát, majd készíts friss előnézetet.

### Nem társított ügyfélüzenetek feldolgozása

Az `Áttekintő` `Ügyfélpostafiók` paneljéről nyisd meg a `Nem társított üzenetek` oldalt. Ide kerül az a beérkezett levél, amelyet a rendszer nem tud egyetlen ügyféllevelezéshez sem biztonságosan hozzárendelni. A bizonytalan automatikus levelek szintén itt maradnak kézi ellenőrzésre; önmagukban nem hoznak létre `Új válasz` állapotot.
...
A revízió egy adott időpont projektállapotának változatlan, kanonikus Markdown-specifikációja. A kiválasztott publikált sablon biztonságos helyőrzőkön keresztül jeleníti meg a projekt-, felmérési, felkészültségi és döntési értékelési adatokat. Nem a projekt élő nézete: egy későbbi adat- vagy sablonmódosítás nem írja át.
...
A nem kötelező sablonblokkok a közvetlenül előttük álló címsorral együtt kimaradnak, ha a hozzájuk tartozó adat még nem elérhető. A kötelező helyőrző hiánya ehelyett az érintett előkészítési adat magyar nevét megadó hibaüzenettel leállítja a generálást. A revízió továbbra sem tartalmazza:

- a tisztázási tételek listáját, döntéseit vagy felelőseit;
- az ügyfél-utánkövetési ütemezést és emlékeztető-állapotot;
- a később, más revízió után beírt adatokat.

Ezért átadáskor a Markdown mellett külön ellenőrizd a `Felkészültség` oldal tisztázási listáját is.

### Az első revízió létrehozása

Revízió nélkül a `Revíziótörténet` rész a `Még nincs Markdown-revízió` állapotot mutatja.
...
5. Válaszd a `Markdown-revízió generálása` gombot.
6. Várd meg, amíg a revízió megjelenik a listában és a `Revízió részletei` betöltődik.
...
A `Becslésre kész` állapotba lépés automatikusan `Mérföldkő elérése` okú revíziót hoz létre. Ezt nem kell még egyszer kézzel megismételni, hacsak egy későbbi adatmódosításról nem akarsz új pillanatképet.
...
| Adat | Jelentés |
| --- | --- |
| `Létrehozva` | Mikor készült a változatlan revízió |
| `Mérföldkő` | A névvel jelölt üzleti ellenőrzési pont, vagy `Nincs` |
| `Forrásverzió` | A revízió saját forráspillanatképének verziója |
| `Sablon` | A generáláskor használt sablon neve és változatlan publikált verziója |
| `Előző revízió` | Link a közvetlen előző revízióhoz, vagy `Első revízió` |
...
- Ha a lista nem töltődik be, válaszd a `Revíziók újratöltése` gombot.
- Ha csak a kijelölt revízió részlete hibás, válaszd a `Revízió részleteinek újratöltése` műveletet vagy nyiss meg másik listatételt.
...
- archiváláskor a projekt, a válaszok, tisztázási tételek, revíziók és belső események megmaradnak;
...
1. Győződj meg róla, hogy nincs koordinációmentés, tisztázási tétel lezárása vagy e-mail-küldés folyamatban.
...
Az archivált projekt megőrzött adatai elérhetők maradnak. A koordináció, az ügyfél-utánkövetési műveletek, valamint a tisztázási tétel létrehozása és lezárása letiltott. A projektoldalak, a megőrzött tartalom és a legutóbbi üzleti aktivitás továbbra is olvasható.

A jelenlegi kiadásban a felmérési és Markdown-oldal közvetlen útvonala archiválás után is megnyitható lehet. Ezt ne értelmezd engedélyként új tartalom létrehozására. A biztonságos szabály: előbb `Projekt visszaállítása`, utána új séma, kör, válasz vagy revízió.

### Visszaállítás

1. Nyisd meg az `Archivált` projektet az Áttekintőről.
2. Válaszd a `Projekt visszaállítása` gombot.
3. Várd meg az `A projekt visszaállt Előkészítés alatt állapotba.` sikerüzenetet.
4. Állítsd be újra a valós aktív státuszt, felelőst, következő lépést és határidőt.

A visszaállítás mindig `Előkészítés alatt` állapotot ad. Nem emlékszik az archiválás előtti aktív állapotra. A megőrzött sémák, körök, tisztázási tételek, revíziók és belső események megmaradnak.
...
- Markdown-revízió;
- tisztázási tétel;
- mentett ügyfél-utánkövetési állapot, amely beállítás vagy emlékeztető során is létrejöhet.
...
| Áttekintő-, projektoldal-, felmérés-, kérdésbank- vagy Markdown-betöltési hiba | A betöltés nem módosít adatot | Válaszd az oldal nevével jelölt `… újratöltése` műveletet. A közös projektfejlécből vagy visszalépő linkkel biztonságosan visszatérhetsz; ismételt hiba esetén jelezd az üzemeltetőnek |
| A projekt nem található | Más projekt nem változik | Térj vissza az `Áttekintő` oldalra. Ellenőrizd, hogy a projektet nem törölték-e, és a listából nyisd meg újra |
| A kiválasztott Markdown-revízió nem található | A többi revízió és projektadat megmarad | Térj vissza a `Revíziótörténethez`, és válassz létező revíziót |
| `409` ütközés vagy elavult oldalállapot | A rendszer az egyik érvényes állapotot megőrizte; az elutasított kérés nem írta felül | Tisztázási tétel szerkesztési ütközésénél ne ezt az általános oldal-újratöltést használd; lásd a következő sort. Más esetben töltsd újra az oldalt, olvasd el a friss állapotot, majd csak szükség esetén ismételd meg a módosítást |
| Tisztázási tétel szerkesztési ütközése | A helyi szerkesztőpiszkozat és a legfrissebb lista megmarad; a régi verziós mentés nem ír felül adatot | Ne töltsd újra általánosan az oldalt, mert ez eldobná a megőrzött piszkozatot. Ha a frissítés sikertelen, válaszd a `Frissítés újrapróbálása` műveletet. Csak sikeres frissítés után válaszd nyitott tételnél az `Aktuális verzió betöltése` gombot, ellenőrizd az új értékeket, majd szükség esetén javítsd és mentsd újra; lezárt tételnél nincs újraszerkesztés vagy újratöltés, csak `Mégse` |
...
| A levélküldés nincs beállítva | Projekt, revízió és utánkövetési állapot nem vész el; engedélyezési vagy küldési kérés sikertelen | Ne ismételd vakon. Kérd az üzemeltetőt a levélküldés ellenőrzésére |
...
| Sikertelen e-mail-küldés | A projekt és a revízió megmarad; biztonságos hibakód rögzül | Ellenőrizd a címzettet és a szolgáltatás állapotát. Csak az ok tisztázása után ismételd meg a megfelelő küldést |
| Az e-mail gomb letiltott módosított utánkövetési űrlap mellett | Semmi nem ment ki | Mentsd az utánkövetési beállítást, vagy állítsd vissza a mezőket a mentett értékre |
...
| A tisztázási tétel már le van zárva | Az első végleges döntés megmarad | Töltsd újra a `Felkészültség` oldalt. Ne hozz létre második lezárást; szükséges új kérdésből készíts új tisztázási tételt |
...
| Áttekintő | Az aktív projektek és következő feladataik közös nézete |
...
| Projektbeállítások | A projekt adminisztratív felülete: alapadatok, ügyfélkapcsolati beállítások, életciklus és veszélyzóna |
| Belső felelős | A projektet vivő, név szerint rögzített PO/PM munkatárs |
| Következő lépés felelőse | A belső felelős vagy az ügyfél kapcsolattartója; a felület mindig a kiválasztott konkrét nevet mutatja |
...
| Tisztázási tétel | Külön számon tartott tisztázandó pont felelőssel, dátummal, következő lépéssel és végleges döntéssel |
| Ügyfél-emlékeztető | E-mail-emlékeztetők automatikus ütemezése és kézi küldési állapota; nem tisztázási munkaelemlista |
| Markdown-revízió | Változatlan, verziózott projekt- és felmérési pillanatkép előnézettel és letöltéssel |
...
| `Felmérés folyamatban` | Aktív igényfelmérés | Minden szöveges válasz `Mentve`? |
| `Belső feladatra vár` | Belső válaszra vagy döntésre vár | Van-e megnevezett belső felelős és dátum? |
| `Ügyfélre vár` | Ügyfélválaszra vár | Helyes-e a címzett, és kell-e emlékeztető? |
| `Becslésre kész` | Kézzel becslésre késznek jelölt | Ellenőrizték-e a legfrissebb Markdown-revíziót? |
...
| Felmérési kör | `Felmérési kör lezárva` | A felmérés véget ért; csak aktív ügyfélcsomag-piszkozat mellett szerkeszthető |
| Ügyfélcsomag | `Piszkozat` | Aktív, szerkeszthető verzió |
| Ügyfélcsomag | `Átadás folyamatban` | Küldési kísérlet folyamatban |
| Ügyfélcsomag | `Átadva a levelezőrendszernek` | Elküldött, változatlan verzió |
| Ügyfélcsomag | `Sikertelen` | Ismert hiba után újrapróbálható |
| Ügyfélcsomag | `Ellenőrzést igényel` | Kézi kézbesítési ellenőrzést igényel |
| Tisztázási tétel | `Nyitott` | Döntésre vagy válaszra vár |
| Tisztázási tétel | `Megválaszolva` | Végleges, érdemi válasz rögzítve |
| Tisztázási tétel | `Nem releváns` | Végleges, az elvetés indoka rögzítve |
...
- A jelenlegi felület az aktuális kezdő felméréshez tartozó ügyfélcsomag-verziókat mutatja; több külön történeti felmérési kör összevont böngészője nincs.
- A felkészültségi sáv nem állít át életciklus-állapotot, és nem helyettesíti a csapat üzleti döntését.
...
### Utánkövetés és kommunikáció

- Tisztázási tétel újranyitása és törlése nem elérhető. Forráskapcsolat a `Felkészültség` oldalon, nyitott tételhez kezelhető.
- Nincs automatikus lejártság-kiemelés a projekten belüli tisztázási listában; a közös `Utánkövetések` oldal határidő szerint rendez.
- A felmérési ügyfélcsomag a projekt kapcsolattartójának küldhető, előnézettel és megerősítéssel; nincs címzett-felülírás, olvasási visszaigazolás vagy szabad levélsablon-szerkesztő.
...
- A Markdown nem tartalmaz tisztázási tételt vagy ügyfél-emlékeztető állapotot.
...
- A visszaállítás nem hozza vissza az archiválás előtti állapotot; mindig `Előkészítés alatt` lesz.
...
- [ ] A helyes projektet nyitottam meg az `Áttekintő` oldalról.
- [ ] Az életciklus-állapot megfelel a valós helyzetnek.
...
- [ ] Az utánkövetési beállítási űrlapon nincs nem mentett módosítás.
- [ ] Ügyfél-emlékeztető esetén a mentett piszkozat és a pontos előnézet aktuális.
- [ ] A legfrissebb revízió verzióját és `Tartalmi előnézet` részét végigolvastam.
...
- [ ] Minden fennmaradó bizonytalanságnak van külön tisztázási tétele.
- [ ] Minden tisztázási tételhez tartozik felelős, valódi dátum és következő lépés.
- [ ] A lezárt tisztázási tételek döntésszövege önmagában érthető.
- [ ] Friss Markdown-revízió készült, és a változásösszefoglaló mellett a teljes előnézetet is ellenőriztem.
...
Ha a fenti ellenőrzőlisták teljesülnek, a következő munkatárs a `Projektállapot`, a `Felkészültség`, az `Ügyféllevelezés`, a `Projektbeállítások` és a Markdown-történet alapján ugyanazt a projektállapotot tudja rekonstruálni, amelyből te befejezted a munkát.
