...
| `DATA-01`, `DATA-02` | Preserve data through forward-only evolution | Stable project, identity, discovery, output, Customer-mail, delivery provenance, resumable archive state, explicit draft deletion, governed attachment ownership, and professional-English successor content survive the retained `0001` → `0036` migration chain. | [supported migration proof](../apps/api/test/supported-migration-sequence.e2e-spec.ts), [migration sequence](../apps/api/test/migration-sequence.spec.ts) |
