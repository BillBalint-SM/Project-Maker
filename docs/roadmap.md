...
| `DATA-02` | Evolve the schema through explicit, versioned migrations with reversible intent | Nine explicit migrations support the delivered slices. Migration `0009` has a guarded rollback that refuses to remove persisted assessment overrides. A durable policy for every future evolution and reversal remains planned and must accompany the relevant schema change. | [requirements](../.planning/REQUIREMENTS.md); [operations handoff](operations-handoff.md); depends on planned schema changes |
