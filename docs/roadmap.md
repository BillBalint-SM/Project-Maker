...
| `DATA-02` | Evolve the schema through explicit, versioned migrations with reversible intent | Twenty-three explicit migrations support the delivered slices. The current sequence retains versioned handoffs and pings, mailbox delta state, inbound Customer messages, correspondence processing, receipt-proven revision history, and unmatched-mail triage; guarded reversal refuses to discard retained communication evidence. A durable policy for every future evolution and reversal remains planned and must accompany the relevant schema change. | [requirements](../.planning/REQUIREMENTS.md); [operations handoff](operations-handoff.md); [supported-baseline migration proof](../apps/api/test/m365-channel-upgrade-migration.e2e-spec.ts) |
