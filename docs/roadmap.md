...
| `DATA-01`, `DATA-02` | Preserve data through forward-only evolution | Stable project, identity, discovery, output, Customer-mail, and delivery provenance survive the retained `0001` → `0032` migration chain. | [supported migration proof](../apps/api/test/supported-migration-sequence.e2e-spec.ts), [migration sequence](../apps/api/test/migration-sequence.spec.ts) |
...
| --- | --- | --- | --- |
| `PROJECT-UX-01` | Remove lifecycle dead ends from normal Project work | Editing and retained archive/download behavior exist, but restore still returns `DRAFT`, not the prior phase; the complete requirement remains open. | [project lifecycle E2E](../apps/api/test/projects.e2e-spec.ts), [project service](../apps/api/src/projects/projects.service.ts) |
