...
| `DATA-01`, `DATA-02` | Preserve data through forward-only evolution | Stable project, identity, discovery, output, Customer-mail, delivery provenance, resumable archive state, and the explicit draft-deletion boundary survive the retained `0001` → `0034` migration chain. | [supported migration proof](../apps/api/test/supported-migration-sequence.e2e-spec.ts), [migration sequence](../apps/api/test/migration-sequence.spec.ts) |
