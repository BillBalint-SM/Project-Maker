...
| --- | --- | --- | --- |
| `SCORE-01.2` | Show Decision Score and recommendation | Unavailable. The accepted scope is a server-derived Decision Review: six nullable 1–5 input ratings with a Score, recommendation, and safe explanation only when the inputs and canonical current readiness are complete. It excludes formal Go/Conditional Go/No-Go recording and stored derived snapshots. | [requirements](../.planning/REQUIREMENTS.md), [product domain](product-domain.md), [ADR-0002](adr/0002-pre-delivery-decision-score-policy-correction.md); depends on SCORE-01.1 |
