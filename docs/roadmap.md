...
| --- | --- | --- | --- |
| `INTAKE-04` | Complete discovery-follow-up management | `INTAKE-04.1`, `INTAKE-04.2`, and `INTAKE-04.3a` deliver creation, review, resolution, and safe open-item editing. `INTAKE-04.3b` optional source checklist-item linkage remains the planned parent scope. Delivered SCORE-01.1 reads follow-up resolution as a readiness input; this work does not replace customer email scheduling or project lifecycle. | [requirements](../.planning/REQUIREMENTS.md); [delivered creation](superpowers/plans/2026-08-08-intake-04-discovery-follow-ups.md); [delivered resolution](superpowers/plans/2026-08-08-intake-04-follow-up-resolution.md); [delivered editing](superpowers/plans/2026-08-10-intake-04-discovery-follow-up-editing.md); depends on project and intake data |
| `SCORE-01.2` | Show Decision Score and recommendation | Unavailable. The immutable playbook retains decision policy only; no calculation, recommendation, or decision-support workflow is available. | [requirements](../.planning/REQUIREMENTS.md); depends on SCORE-01.1 |
...
| `DATA-02` | Evolve the schema through explicit, versioned migrations with reversible intent | Ten explicit migrations support the delivered slices. Migration `0009` has a guarded rollback that refuses to remove persisted assessment overrides; migration `0010` reversibly aligns database text-answer whitespace validation with the API. A durable policy for every future evolution and reversal remains planned and must accompany the relevant schema change. | [requirements](../.planning/REQUIREMENTS.md); [operations handoff](operations-handoff.md); depends on planned schema changes |
