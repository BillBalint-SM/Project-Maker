...
| `DATA-02` | Evolve the schema through explicit, versioned migrations with reversible intent | Eight explicit migrations support the delivered slices. A durable policy for every future evolution and reversal remains planned and must accompany the relevant schema change. | [requirements](../.planning/REQUIREMENTS.md); depends on planned schema changes |
