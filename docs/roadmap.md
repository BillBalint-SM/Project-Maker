...
| --- | --- | --- | --- |
| `ATTACH-01` | Retain governed discovery attachments where employees use them | No retained file-content module exists. The accepted design adds versioned Question Bank reference files and Project work attachments on Initial Intake checklist snapshots and Discovery follow-ups, with bounded validation, malware scanning, safe download, lifecycle enforcement, and recovery coverage. It does not create a general document library or retain Customer inbound attachment content from the Correspondence mailbox. | [accepted design](attach-01-governed-discovery-attachments.md); implements a `DATA-02` migration slice, advances `DATA-01`/`DATA-03`, and remains subject to the `SEC-01` exposure gate |
