...
| `INTAKE-02` | Run one reliable initial intake | `INITIAL_INTAKE` question snapshots, answer recording, active-round recovery, and durable meeting-end state | [guided-intake plan](superpowers/plans/2026-08-06-guided-intake-persistence.md), [round E2E](../apps/api/test/question-rounds.e2e-spec.ts), `8ae4e4e` |
...
| `COMM-01` | Send controlled customer communication | Versioned interview handoff plus authored, previewed and optionally referenced customer follow-up ping delivery through Microsoft 365. Ping preview binds an exact `@pte.hu` sender; every logical delivery owns immutable outbound, correspondence and central Reply-To identities. Manual and PostgreSQL-coordinated scheduled attempts survive reload, preserve identity across explicit retry, retain `FAILED`/`UNKNOWN` recovery semantics, and never retry automatically. Claude Markdown is never sent to customers. | [operations handoff](operations-handoff.md), [follow-up routes](../apps/api/src/follow-ups/follow-up.controller.ts), [API E2E](../apps/api/test/customer-follow-up-ping.e2e-spec.ts), [browser E2E](../apps/web/e2e/customer-follow-up-ping.spec.ts) |
| `DOC-01` | Teach employees every stable delivered workflow | Hungarian business-functional guide for all current routes, actions, states, side effects, recovery branches, and limitations, supported by nine sanitized screenshots and three workflow diagrams | [user guide](user-guide.md), [design](superpowers/specs/2026-08-09-user-guide-design.md), [implementation plan](superpowers/plans/2026-08-09-user-guide.md), [guided-intake E2E](../apps/web/e2e/guided-intake.spec.ts), [discovery E2E](../apps/web/e2e/discovery-follow-ups.spec.ts), [readiness E2E](../apps/web/e2e/readiness-review.spec.ts), [deletion E2E](../apps/web/e2e/project-delete.spec.ts) |
...
| `DATA-02` | Evolve the schema through explicit, versioned migrations with reversible intent | Fourteen explicit migrations support the delivered slices. The latest migration adds named ownership, the `OPEN`/`ENDED` meeting lifecycle, versioned customer handoffs, and guarded reversal while retained handoff data exists. A durable policy for every future evolution and reversal remains planned and must accompany the relevant schema change. | [requirements](../.planning/REQUIREMENTS.md); [operations handoff](operations-handoff.md); depends on planned schema changes |
