...
| `DATA-02` | Evolve the schema through explicit, versioned migrations with reversible intent | Thirteen explicit migrations support the delivered slices. The latest migrations add Decision Review inputs and the Markdown template library with guarded reversal when retained activity exists. A durable policy for every future evolution and reversal remains planned and must accompany the relevant schema change. | [requirements](../.planning/REQUIREMENTS.md); [operations handoff](operations-handoff.md); depends on planned schema changes |
